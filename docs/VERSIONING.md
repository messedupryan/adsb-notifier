...
- Avoid deploying `latest` for normal testing. Use the explicit version tag, such as `0.0.13`.
...
```text
registry.example.test/adsb-notifier-worker:0.0.13
registry.example.test/adsb-notifier-api:0.0.13
registry.example.test/adsb-notifier-ui:0.0.13
```
